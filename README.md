...
기능적 요구사항
1. 회원 관리
    1. 회원 등록(ID, Password, 전화번호)
        - 회원 등록 시 Password, 전화번호는 암호화
    1. 전화 번호로 ID, Password 찾기
    1. 회원 탈퇴
1. 콘텐츠 관리  
    1. 회원은 본인의 콘텐츠를 등록/조회/수정/삭제/Best를 할 수 있다.
    1. 회원은 타인의 콘텐츠를 조회/Best를 할 수 있다.
    1. 콘텐츠의 정보는 제목, 내용(동영상), Tag(카테고리), 조회수, Best로 구성된다.
        - 콘텐츠 조회 시 사용 가능 여부 확인
    1. 제목 또는 Tag로 검색할 수 있다.
1. 추천 시스템
    1. 유저 별 추천 동영상 리스트를 제공한다.

비기능적 요구사항
1. 트랜잭션
    1. ---  Sync 호출 
1. 장애격리
    1. 추천 시스템이 Down 되면 Best가 많은 콘텐츠를 가져온다.  
